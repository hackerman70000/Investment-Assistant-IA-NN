...
class training:
    def __init__(self, interval, price_delta, num_intervals):
        self.interval = interval
        self.price_delta = price_delta
        self.num_intervals = num_intervals
        self.timestamp = datetime.now().strftime("%Y-%m-%d-%H-%M-%S")
        self.dir_name = 'saved_models/model_' + self.timestamp
    ...
    def preprocess_market_data(self):
        ...
        df = pd.read_csv(f'training_data/market_data_{self.interval}.csv')
    ...
    def scale_data(self, x_train, x_test, x_dev):
        ...
        scaler.fit(x_train)
        joblib.dump(scaler, self.dir_name + '/scaler.joblib')
    ...
    def apply_pca(self, x_train_scaled, x_test_scaled, x_dev_scaled):
        ...
        joblib.dump(pca, self.dir_name + '/pca.joblib')
    ...
    def evaluate_model(self, model, x_test_PCA, y_test):
        ...
        model.save(self.dir_name + '/NN_' + str(round(evaluation[1] * 100, 1)) + '%' + '.h5')
    ...
    def save_results(self, model, x_dev_PCA, y_dev, evaluation):
        ...
        predictions.to_csv(self.dir_name + '/predictions.csv', index=False)
        ...
        with open(self.dir_name + '/results.txt', 'w') as f:
        ...
        try:
            ...
            os.rename(self.dir_name, final_dir_name + '_' + self.timestamp)
        except OSError as e:
            print(f"Error: {e}")
...
if __name__ == '__main__':
    training(interval="1h", price_delta=1.012, num_intervals=12).run()
