...
class Scraper:
    ...
    def __init__(
        ...
        symbol: str = "BTCUSDT",
        interval: str = "1d",
        start_date: datetime = EARLIEST_DATE,
    ...
    def validate_inputs(self) -> None:
        ...
        if self.start_date < self.EARLIEST_DATE:
            logging.error(
                f"Invalid start date: {self.start_date}. Data is only available from {self.EARLIEST_DATE}"
            )
            raise ValueError(
                f"Invalid start date: {self.start_date}. Data is only available from {self.EARLIEST_DATE}"
            )
        ...
        if self.verify_checksum:
            ...
            if checksum_content:
                checksum_file_path = os.path.join(
                    self.directory, os.path.basename(checksum_url)
                )
    ...
    def run(self) -> None:
        ...
        logging.info(
            f"Starting data scraping for {self.symbol} from {self.start_date} to {self.end_date}"
        )
        with ThreadPoolExecutor(max_workers=5) as executor:
            future_to_url = {
                executor.submit(self.download_and_process_url, url): url for url in urls
            }
            for future in tqdm(
...
def parse_arguments() -> argparse.Namespace:
    ...
    parser.add_argument(
        "--directory", type=str, default="data/raw", help="Directory to save data"
    )
    parser.add_argument(
        "--verify_checksum",
        action="store_true",
        help="Verify checksum of downloaded files",
    )
    parser.add_argument(
        "--log_file", type=str, default="logs/scrape.log", help="Path to log file"
    )
